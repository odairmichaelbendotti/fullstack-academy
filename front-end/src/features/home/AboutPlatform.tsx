...
function AccordionRow({ item, isActive, onToggle }: {
  ...
  return (
    <div className={`relative border-b border-outline transition-colors duration-300 ${isActive ? "bg-surface/40" : ""}`}>
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-4 px-5 md:px-10 py-4 md:py-5 text-left group cursor-pointer"
      >
        ...
        <span className={`flex-1 text-sm md:text-base font-semibold transition-colors duration-200 ${isActive ? "text-textPrimary" : "text-textSecondary group-hover:text-textPrimary"}`}>
          {item.title}
        </span>
      ...
      <div style={{ height, overflow: "hidden", transition: "height 0.35s cubic-bezier(0.4,0,0.2,1)" }}>
        <div ref={contentRef} className="px-5 md:px-10 pb-5 md:pb-6">
          ...
          {item.cta && (
            <Link
              href={item.cta.href}
              className="inline-flex items-center gap-1.5 mt-3 md:mt-4 text-sm font-semibold text-primary hover:text-secondary transition-colors duration-200 group/cta"
            >
              {item.cta.text}
...
const AboutPlatform = () => {
  ...
  return (
    <section className="relative w-full border-t border-outline flex flex-col md:flex-row">

      {/* ── COLUNA ESQUERDA — acordeão ── */}
      <div className="flex-1 flex flex-col min-w-0">
        <div className="px-5 md:px-10 pt-8 md:pt-10 pb-6 md:pb-8 border-b border-outline">
          <span
            className="inline-flex items-center px-3 py-1 mb-3 md:mb-4 rounded-full border border-primary text-primary text-[10px] md:text-[11px] font-bold tracking-widest uppercase"
            style={{ background: "rgba(245,158,11,0.12)" }}
          >
            O que é a Fullstack Academy?
          </span>
          <h2 className="text-xl md:text-3xl font-bold text-textPrimary leading-snug">
      ...
      {/* ── COLUNA DIREITA — imagem full-height, só desktop ── */}
